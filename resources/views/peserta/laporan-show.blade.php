    ...
    <div class="p-6 card shadow-soft md:p-8">
        ...
        <div class="space-y-6">
            <div>
                ...
                <div class="p-4 border rounded-lg bg-slate-50 border-slate-200">
                    <p class="text-sm leading-relaxed text-slate-700 whitespace-pre-wrap">{{ $laporan->deskripsi }}</p>
                </div>
            </div>
        ...
        @if($laporan->status == 'Draft' || $laporan->status == 'Revisi')
            <div class="flex justify-end gap-3 pt-6 mt-6 border-t border-slate-200">
                <form action="{{ route('laporan.destroy', $laporan->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus laporan ini?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
