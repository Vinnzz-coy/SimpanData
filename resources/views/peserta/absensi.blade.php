        ...
        <div class="px-4 py-2 border border-blue-100 bg-blue-50 rounded-xl animate-fade-in" style="animation-delay: 200ms">
            ...
            <p class="text-sm font-extrabold text-blue-900">
                @if($todayAttendance->isEmpty())
                    Belum Presensi
                @elseif($todayAttendance->where('jenis_absen', 'Masuk')->count() > 0 && $todayAttendance->where('jenis_absen', 'Pulang')->count() == 0)
                    Sudah Masuk
                @else
                    Sudah Pulang
                @endif
                ...
                <div>
                    ...
                    <div id="hadir-types-container" class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <button type="button" id="checkout-btn"
                                class="p-4 transition-all duration-200 border-2 border-slate-100 rounded-2xl hover:border-red-400 hover:bg-red-50/30 group focus:outline-none">
                            <div class="flex items-center justify-center gap-3">
